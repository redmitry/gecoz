...
public class Sequence {
    final static char[] SEQ = {'=','A','C','M','G','R','S','V','T','W','Y','H','K','D','B','N'};
    ...
    protected Sequence(byte[] sequence, int l_seq) {
        seq = sequence;
        this.l_seq = l_seq;
    }
    ...
    public static String parse(byte[] sequence, int l_seq) {
        char[] arr = new char[sequence.length * 2];
        for (int i = 0, idx = 0; i < sequence.length; i++) {
            arr[idx++] = SEQ[sequence[i] >> 4 & 0x0F];
            arr[idx++] = SEQ[sequence[i] & 0x0F];
        }
        return new String(arr, 0, l_seq);        
    }
